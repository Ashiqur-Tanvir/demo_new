...
/**
 * Diese Klasse modelliert eine Entitaet, die sich ueber das Spielraster bewegen
 * kann. Modelliert ausserdem eine "Blickrichtung".
 *
...
public abstract class MovingEntity extends GameEntity {
	public static final String[] saveStateOrder = { "start", "target", "speed", "lastMoveTick", "moveIntervall",
			"moveSpeedX", "moveSpeedY", "dir" };
	...
	/**
	 * Konstruktor. Schaue in Richtung 0.
	 */
	public MovingEntity() {
		super();
		dir = 0;
	...
	/**
	 * Konstruktor. Alle Daten werden aus dem uebergebenen Stream gelesen.
	 *
	...
	public void writeEntity(DataOutputStream s) throws IOException {
		 ...
		 */
		s.writeBoolean(this.start == null);
		 ...
		 * Überprüfung auf null wird erst gespeichert. Wenn nicht null, auch die Werte
		 */
		s.writeBoolean(this.target == null);
	...
	/**
	 * {@inheritDoc} Verschiebt ausserdem die geplante Bewegung, diese wird NICHT
	 * abgebrochen.
	 *
	...
	/**
	 * Aendert die Blickrichtung anhand der geplanten Bewegung. 0: rechts, 1: oben;
	 * 2: links; 3: unten
	 *
	...
	public void move(Vector2f pos) {
		...
		moveInterval = (int) (Math.sqrt(
				(target.x - start.x) * (target.x - start.x) + (target.y - start.y) * (target.y - start.y)) / speed); // =
																														// distanz/speed
	...
	/**
	 * Beendet die aktuelle fluessige Bewegung sofort. Die Entitaet wird zur
	 * Zielposition bewegt.
	 */
	public void endMove() {
		if (target != null) {
	...
	/**
	 * Beendet die aktuelle fluessige Bewegung sofort. Die Entitaet wird NICHT zur
	 * Zielposition bewegt.
	 */
	...
	/**
	 * {@inheritDoc} Aktualiesiert die Position anhand der geplanten Bewegung.
	 */
	...
	/**
	 * Laesst die Entitaet auf die andere Seite des Levels teleportieren. Wird nur
	 * ausgefuehrt, wenn die Position ausserhalb der Levelgrenzen ist.
	 *
