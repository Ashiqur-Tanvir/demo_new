...
public class GamePlayState extends BasicGameState {
    ...
    // TODO task 5a
    /**
	 * Die Wände werden miteinander verbunden
	 * 
	 * @param level Level
	 */
	private void computeTextures(Level level) {
		...
		// Array mit Texturangaben wird initialisiert
		neededTexture = new int[level.getHeight()][level.getWidth()];

		/*
		 * Hier werden alle 16 Varianten für eine Wandverbindung als Binärzahlen in der
		 * Array gespeichert. Falls keine Wand vorhanden, wird 0 gespeichert
		 */
		for (int y = 0; y < level.getHeight(); y++) {
			for (int x = 0; x < level.getWidth(); x++) {
				if (level.isWall(x, y)) {
					boolean a = level.isWall(x - 1, y); // Überprüfe linke Zelle
					boolean b = level.isWall(x, y + 1); // Überprüfe untere Zelle
					boolean c = level.isWall(x + 1, y); // Überprüfe rechte Zelle
					boolean d = level.isWall(x, y - 1); // Überprüfe obere Zelle

					if (a) {
						if (b) {
							if (c) {
								if (d) {
									neededTexture[y][x] = 0b1111;
								} else
									neededTexture[y][x] = 0b1110;
							} else {
								if (d) {
									neededTexture[y][x] = 0b1101;
								} else {
									neededTexture[y][x] = 0b1100;
								}
							}
						} else {
							if (c) {
								if (d) {
									neededTexture[y][x] = 0b1011;
								} else {
									neededTexture[y][x] = 0b1010;
								}
							} else {
								if (d) {
									neededTexture[y][x] = 0b1001;
								} else {
									neededTexture[y][x] = 0b1000;
								}
							}
						}
					} else {
						if (b) {
							if (c) {
								if (d) {
									neededTexture[y][x] = 0b111;
								} else {
									neededTexture[y][x] = 0b110;
								}
							} else {
								if (d) {
									neededTexture[y][x] = 0b101;
								} else {
									neededTexture[y][x] = 0b100;
								}
							}
						} else {
							if (c) {
								if (d) {
									neededTexture[y][x] = 0b11;
								} else {
									neededTexture[y][x] = 0b10;
								}
							} else {
								if (d) {
									neededTexture[y][x] = 0b1;
								} else {
									neededTexture[y][x] = 0b0;
								}
							}
						}
					}
				} else
					neededTexture[y][x] = 0b0000;
			}
		}
